...
const Step1_InitialPrompt: React.FC<Pick<VideoGeneratorProps, 'prompt' | 'setPrompt' | 'negativePrompt' | 'setNegativePrompt' | 'image' | 'setImage' | 'onGenerateScript' | 'isScriptLoading' | 'duration' | 'setDuration' | 'aspectRatio' | 'setAspectRatio' | 'numberOfVersions' | 'setNumberOfVersions' | 'withWatermark' | 'setWithWatermark' | 'videoType' | 'setVideoType'>> = ({
    ...
    return (
        <div className="bg-white rounded-xl p-8 shadow-lg border border-gray-200">
            <h3 className="text-2xl font-bold text-bf-slate mb-6 font-serif">{t('videoGenerator.step1Title')}</h3>
            <form onSubmit={handleSubmit} className="space-y-6">
                <div>
                    <label className="block text-sm font-bold uppercase tracking-wide text-gray-700">{t('videoGenerator.videoType')}</label>
                     <div className="mt-2 grid grid-cols-2 gap-3">
                        <label className={`cursor-pointer rounded-md p-3 text-center border-2 transition-colors ${videoType === 'general' ? 'border-bf-orange bg-orange-50' : 'border-gray-200 bg-white hover:bg-gray-50'}`}>
                            ...
                            <span className="text-sm font-bold text-gray-800">{t('videoGenerator.typeGeneral')}</span>
                        </label>
                        <label className={`cursor-pointer rounded-md p-3 text-center border-2 transition-colors ${videoType === 'research_showcase' ? 'border-bf-orange bg-orange-50' : 'border-gray-200 bg-white hover:bg-gray-50'}`}>
                            ...
                            <span className="text-sm font-bold text-gray-800">{t('videoGenerator.typeBooth')}</span>
                        </label>
                    </div>
                </div>
                <div>
                    <label htmlFor="video-prompt" className="block text-sm font-bold uppercase tracking-wide text-gray-700">{t('videoGenerator.promptLabel')}</label>
                    <textarea
                        id="video-prompt"
                        rows={5}
                        value={prompt}
                 ...
                 <div>
                    <label htmlFor="video-negative-prompt" className="block text-sm font-bold uppercase tracking-wide text-gray-700">{t('videoGenerator.negativePromptLabel')}</label>
                    <textarea
                ...
                <div>
                    <label className="block text-sm font-bold uppercase tracking-wide text-gray-700">{t('videoGenerator.imageLabel')}</label>
                    <div className="mt-1 flex flex-col items-center justify-center p-4 border-2 border-gray-300 border-dashed rounded-md space-y-3 bg-gray-50">
                        <label htmlFor="image-upload" className="relative cursor-pointer bg-white border border-gray-300 rounded-md font-bold text-bf-orange hover:text-bf-orange-dark focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-bf-orange px-4 py-2 transition-colors">
                ...
                <div className="space-y-4 pt-4 border-t border-gray-200">
                        ...
                        <div className="relative flex items-start">
                            <div className="flex h-6 items-center">
                                <input
                                    ...
                                    className="h-4 w-4 rounded border-gray-300 text-bf-orange focus:ring-bf-orange cursor-pointer"
                                    disabled={isScriptLoading}
                                />
                            </div>
                            <div className="ml-3 text-sm leading-6">
                                <label htmlFor="with-watermark" className="font-bold text-gray-700 cursor-pointer">
                    ...
                    <div>
                        <label className="block text-sm font-bold uppercase tracking-wide text-gray-700">{t('videoGenerator.numberOfVersions')}</label>
                        <div className="mt-2 grid grid-cols-2 gap-3">
                            {[1, 2].map(num => (
                     ...
                     <div>
                        <label className="block text-sm font-bold uppercase tracking-wide text-gray-700">{t('videoGenerator.aspectRatio')}</label>
                    ...
                    <div>
                        <label htmlFor="video-duration" className="block text-sm font-bold uppercase tracking-wide text-gray-700">
                        ...
                        <input
                            ...
                            className="mt-2 block w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-bf-orange"
                            disabled={isScriptLoading}
                        />
                    </div>
                </div>
                ...
                <div>
                    <button type="submit" disabled={isScriptLoading} className="w-full flex justify-center py-4 px-4 border border-transparent rounded-full shadow-md text-sm font-bold uppercase tracking-wide text-white bg-gradient-to-r from-blue-600 via-purple-700 to-pink-700 hover:from-blue-700 hover:to-pink-800 disabled:bg-gray-400 disabled:cursor-not-allowed transition-all">
...
const SceneItem: React.FC<{
    ...
    return (
        <div className="p-6 bg-white rounded-xl border border-gray-200 shadow-sm grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
            {/* Left Column: Text and Prompts */}
            <div className="space-y-4">
                <div className="flex justify-between items-center">
                    <h4 className="text-xl font-bold text-bf-slate font-serif">{t('videoGenerator.scene')} {index + 1}</h4>
                    <button 
                        onClick={() => onApproveScene(index, !scene.isApproved)} 
                        disabled={!hasMedia || scene.isGenerating}
                        className={`px-4 py-1.5 text-sm font-bold uppercase tracking-wide rounded-full transition-colors flex items-center justify-center ${scene.isApproved ? 'bg-green-600 text-white' : 'bg-gray-100 hover:bg-gray-200 text-gray-600'} disabled:bg-gray-100 disabled:text-gray-400 disabled:cursor-not-allowed`}
                    >
                        {scene.isApproved && (
                            <svg
                ...
                <div>
                    <label htmlFor={`narration-${index}`} className="block text-sm font-bold uppercase tracking-wide text-gray-700 flex items-center mb-1">
                        {t('videoGenerator.narration')}
                        <button onClick={handleReadNarration} className="ml-2 text-bf-orange hover:text-bf-orange-dark" title={t('videoGenerator.readNarration')}>
                    ...
                    <p id={`narration-${index}`} className="mt-1 text-gray-700 bg-gray-50 border border-gray-200 p-3 rounded-md text-sm italic leading-relaxed">{scene.narration}</p>
                </div>
                <div>
                    <div className="flex justify-between items-center mb-1">
                        <label htmlFor={`description-${index}`} className="block text-sm font-bold uppercase tracking-wide text-gray-700">{t('videoGenerator.visuals')}</label>
                        <button 
                            onClick={() => onConfirmScene(index, !scene.isConfirmed)} 
                            disabled={scene.isGenerating}
                            className={`px-3 py-1 text-xs font-bold uppercase tracking-wide rounded-full transition-colors ${scene.isConfirmed ? 'bg-gray-100 hover:bg-gray-200 text-gray-600' : 'bg-pink-600 hover:bg-pink-700 text-white'}`}
                        >
                    ...
                    <textarea 
                        ...
                        className="block w-full bg-white border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-pink-500 focus:border-pink-500 sm:text-sm text-gray-900 disabled:bg-gray-100 disabled:text-gray-500" 
            ...
            <div className="space-y-4">
                <div className="aspect-video bg-gray-100 rounded-lg flex items-center justify-center relative overflow-hidden border border-gray-200">
                    ...
                    {hasMedia ? (
                           ...
                           <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 w-full h-full">
                                {scene.videoUrls.map((url, i) => (
                                    <div key={i} className="relative group w-full h-full">
                                        ...
                                        <button onClick={() => handleDownload(url, 'video')} className="absolute top-2 right-2 bg-white/80 text-gray-800 p-2 rounded-full opacity-0 group-hover:opacity-100 transition-opacity shadow-sm" title={t('videoGenerator.downloadVideo')}>
                            ...
                            <div className="relative group w-full h-full">
                                ...
                                <button onClick={() => handleDownload(scene.imageUrl!, 'image')} className="absolute top-2 right-2 bg-white/80 text-gray-800 p-2 rounded-full opacity-0 group-hover:opacity-100 transition-opacity shadow-sm" title={t('videoGenerator.downloadVideo')}>
                                     <svg className="w-4 h-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M3 17a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zm3.293-7.707a1 1 0 011.414 0L9 10.586V3a1 1 0 112 0v7.586l1.293-1.293a1 1 0 111.414 1.414l-3 3a1 1 0 01-1.414 0l-3-3a1 1 0 010-1.414z" clipRule="evenodd" /></svg>
                                </button>
                            </div>
                        )
                        ...
                        <div className="text-center p-6">
                            ...
                            <span className="text-gray-400 text-sm font-medium">Visuals will appear here</span>
                        </div>
                    )}
                </div>
                ...
                <div className="space-y-2">
                    {scene.error && !isQuotaErrorInScene && (
                        <div className="text-red-600 text-xs p-3 bg-red-50 rounded-md border border-red-200">
                    ...
                    {isQuotaErrorInScene && (
                        <div className="text-yellow-700 text-xs p-3 bg-yellow-50 rounded-md border border-yellow-200 space-y-3">
                            <p>{t('videoGenerator.quotaErrorImageFallback')}</p>
                            <div className="flex flex-col space-y-2">
                                <button onClick={() => onGenerateSceneVideo(index, true)} className="w-full text-center text-xs bg-yellow-600 text-white font-bold py-2 px-3 rounded-md hover:bg-yellow-700 transition-colors">{t('videoGenerator.generateAlternativeVideo')}</button>
                                <button onClick={() => onGenerateSceneImage(index)} className="w-full text-center text-xs bg-slate-600 text-white font-bold py-2 px-3 rounded-md hover:bg-slate-500 transition-colors">{t('videoGenerator.generateAnimatedScene')}</button>
                                {image && (
                                    <div className="pt-2 border-t border-yellow-200">
                                        <div className="relative">
                                            <input type="text" value={babaPrompt} onChange={(e) => setBabaPrompt(e.target.value)} placeholder={t('videoGenerator.askGoogleBabaFocus')} className="w-full bg-white border border-gray-300 text-xs rounded-md py-2 px-3 focus:ring-pink-500 focus:border-pink-500" />
                                            <button onClick={() => onAskGoogleBaba(babaPrompt)} className="absolute right-1 top-1/2 -translate-y-1/2 bg-blue-600 text-white text-xs px-2 py-1 rounded hover:bg-blue-700">{t('videoGenerator.askGoogleBaba')}</button>
                                        </div>
                                    </div>
                                )}
                            </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <button onClick={() => onGenerateSceneVideo(index)} disabled={!scene.isConfirmed || scene.isGenerating || isQuotaExhausted} className="w-full text-center text-sm bg-gradient-to-r from-blue-600 to-purple-700 text-white font-bold py-2 px-3 rounded-md hover:from-blue-700 hover:to-purple-800 transition-all disabled:from-gray-300 disabled:to-gray-400 disabled:cursor-not-allowed">
                            {hasVideo ? t('videoGenerator.regenerateScene') : t('videoGenerator.generateSceneVideo')}
                        </button>
                        <button onClick={() => onGenerateSceneImage(index)} disabled={!scene.isConfirmed || scene.isGenerating} className="w-full text-center text-sm bg-slate-600 text-white font-bold py-2 px-3 rounded-md hover:bg-slate-500 transition-colors disabled:bg-gray-300 disabled:cursor-not-allowed">
    ...
    return (
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
            <div className="text-center">
                ...
                <p className="mt-4 text-lg text-gray-600 max-w-2xl mx-auto">{t('videoGenerator.subtitle')}</p>
            </div>
            
            { isQuotaExhausted && (
                <div className="mt-8 p-4 bg-red-50 border border-red-200 text-red-600 text-center rounded-md">
                    ...
                    <div className="animate-fade-in space-y-12">
                        <div>
                            <div className="flex justify-between items-center mb-6 flex-wrap gap-4">
                                <h2 className="text-3xl font-bold text-bf-slate font-serif">{t('videoGenerator.step2Title')}</h2>
                                <div className="flex items-center space-x-4">
                                    <span className="text-sm text-gray-500 italic">{t('videoGenerator.progressSavedAutomatically')}</span>
                                    <button onClick={onClear} className="px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm font-bold rounded-md transition-colors">{t('videoGenerator.startOver')}</button>
                                </div>
                            </div>
                        ...
                        <div className="bg-white rounded-xl p-8 shadow-lg border border-gray-200">
                             <h3 className="text-2xl font-bold text-bf-slate mb-6 font-serif">{t('videoGenerator.step3Title')}</h3>
                             <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                 <div className="space-y-4">
                                     <label htmlFor="music-prompt" className="block text-sm font-bold uppercase tracking-wide text-gray-700">{t('videoGenerator.musicPromptLabel')}</label>
                                     ...
                                     <button onClick={onGenerateMusic} disabled={isMusicLoading} className="w-full flex justify-center py-3 px-4 border border-transparent rounded-full shadow-md text-sm font-bold uppercase tracking-wide text-white bg-gradient-to-r from-blue-600 via-purple-700 to-pink-700 hover:from-blue-700 hover:to-pink-800 disabled:bg-gray-400 transition-colors">
                                         {isMusicLoading ? t('videoGenerator.generatingMusic') : t('videoGenerator.generateMusicButton')}
                                     </button>
                                     {musicDescription && (
                                         <div className="mt-4 p-4 bg-gray-50 rounded-md animate-fade-in border border-gray-200">
                                             <h4 className="font-bold text-pink-600 mb-2 uppercase text-xs tracking-wide">{t('videoGenerator.musicDescriptionTitle')}</h4>
                                             <div className="prose prose-sm prose-slate text-gray-600 whitespace-pre-wrap leading-relaxed" dangerouslySetInnerHTML={{ __html: musicDescription.replace(/\n/g, '<br />') }} />
                                         </div>
                                     )}
                                 </div>
                                 <div className="space-y-4">
                                     <h4 className="block text-sm font-bold uppercase tracking-wide text-gray-700">{t('videoGenerator.musicLibraryTitle')}</h4>
                                     <ul className="space-y-2">
                                         {MUSIC_LIBRARY.map(track => (
                                             <li key={track.url} className="flex items-center justify-between bg-gray-50 p-3 rounded-md border border-gray-200">
                                                 <span className="text-sm font-medium text-gray-700">{track.name}</span>
                                                 <div className="flex items-center space-x-2">
                                                     <audio src={track.url} controls className="h-8" />
                                                     <button onClick={() => onSelectMusicUrl(track.url)} className={`px-3 py-1 text-xs font-bold uppercase tracking-wide rounded-full ${selectedMusicUrl === track.url ? 'bg-pink-600 text-white' : 'bg-gray-200 hover:bg-gray-300 text-gray-700'}`}>
                         ...
                         <div className="bg-white rounded-xl p-8 shadow-lg border border-gray-200 text-center">
                             <h3 className="text-2xl font-bold text-bf-slate mb-6 font-serif">{t('videoGenerator.step4Title')}</h3>
                             <button disabled={approvedCount !== scenes.length || !selectedMusicUrl} className="w-full max-w-md mx-auto flex justify-center py-4 px-6 border border-transparent rounded-full shadow-lg text-lg font-bold text-white bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-pink-500 disabled:bg-gray-300 disabled:text-gray-500 disabled:cursor-not-allowed transition-colors uppercase tracking-wide">
